<template>
  <div class="app-container">
    ...
    <!-- 手动检查结果 -->
    <div v-if="showManualSection && manualErrors.length > 0" class="manual-results">
      <h3>
        <span class="result-icon">🔍</span>
        手动检查发现 {{ manualErrors.length }} 处可能错误
      </h3>
      <div class="error-scroll-container">
        <div
            v-for="(error, index) in manualErrors"
            :key="index"
            class="error-item manual-error-item"
            @click="scrollToPosition(error.position)"
        >
          ...
          <div class="error-context">
            {{ getErrorContext(error.position, error.errorWord.length) }}
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="originText" class="compare-container">
      ...
      <div class="doc-panel origin-panel">
        <div class="panel-header">
          <h3>
            ...
            <span v-if="showManualSection && manualErrors.length > 0" class="error-count">
              手动发现{{ manualErrors.length }}处错误
            </span>
      ...
      <!-- 手动修正后区域 -->
      <div class="doc-panel correct-panel" v-if="showManualSection && manualCorrectedText">
        <div class="panel-header">
          <h3>
            <span class="panel-icon">✏️</span>
            手动修正后文档
            <span v-if="manualErrors.length > 0" class="error-count">
              修正{{ manualErrors.length }}处错误
            </span>
          </h3>
          <div class="panel-actions">
            <button @click="copyText(manualCorrectedText)" class="action-btn" title="复制修正文本">
              📋
            </button>
            <button @click="exportText(manualCorrectedText, '手动修正后')" class="action-btn" title="导出修正文本">
              ⬇️
            </button>
            <button @click="applyManualCorrections" class="action-btn apply-btn" title="应用手动修正">
              ✅
            </button>
          </div>
        </div>
        <div class="text-content">
          <div class="text-line" v-for="(line, lineIndex) in manualCorrectedTextLines" :key="lineIndex">
            <span
                v-for="(charData, charIndex) in line.chars"
                :key="charIndex"
                :class="[
...
// 手动修正后的文本行
const manualCorrectedTextLines = computed(() => {
  if (!manualCorrectedText.value || !showManualSection.value) return []

  // 生成修正位置映射
  const correctionMap = generateAccurateCorrectionMap(manualErrors.value, manualCorrectedText.value)

  const lines = manualCorrectedText.value.split('\n')
  const result = []
    ...
    if (extension === 'txt' || extension === 'md') {
      ...
      if (extension === 'doc' && detectedFormat === 'docx') {
        ...
        try {
          ...
          const response = await axios.post('http://localhost:3002/api/check/ai?parse=true', formData, {
            headers: {
  ...
  try {
    ...
    if (fileInput.files.length > 0) {
      ...
      if (['doc', 'docx', 'pdf'].includes(extension)) {
        processingText.value = '使用后端AI检查...'

        const formData = new FormData()
        formData.append('file', file)

        const response = await axios.post('http://localhost:3002/api/check/ai', formData, {
        ...
        if (response.data.success && response.data.data) {
          ...
          if (data.correctedText) {
            originText.value = cleanText(data.correctedText)
          } else if (data.originalText) {
            originText.value = cleanText(data.originalText)
          }

          // 显示错误
          if (data.errors && data.errors.length > 0) {
            aiErrors.value = data.errors.map(error => ({
              ...error,
              isSelected: true
            }))
            selectedCorrectedText.value = data.correctedText || originText.value
            errorMsg.value = `✅ 后端AI识别出 ${aiErrors.value.length} 处错别字`
          } else {
            errorMsg.value = '✅ 未发现错别字'
            selectedCorrectedText.value = originText.value
          }
    ...
    // 对于纯文本，结合后端规则检查和Ollama AI检查
    try {
      // 1. 先调用后端规则检查
      const ruleResponse = await axios.post('http://localhost:3002/api/check/manual', {
        text: originText.value
      })
      if (ruleResponse.data.success) {
        backendErrors = ruleResponse.data.data.errors || []
        console.log(`后端规则检查发现 ${backendErrors.length} 处错误`)
      }
    } catch (e) {
      console.error('后端规则检查失败:', e)
    }

    // 2. 调用Ollama进行AI检查
      const promptText = `请检查以下中文文本中的错别字、用词不当和语法错误。请严格按照要求：
1. 保持原文的换行格式和段落结构
2. 找出所有错误并给出修正建议
3. 返回JSON格式的数据

重要要求：
- "correctText"字段必须是修正后的完整文本，保持原文的换行和段落
- "errorWord"必须是原文中确切的错误词，不要修改大小写或标点
- "correctWord"是对应的正确词
- "position"是错误词在原文中的起始位置（从0开始计算）
...
请返回JSON格式：
{
  "correctText": "修正后的完整文本，保持原格式",
  "errors": [
    {
      "position": 数字,
      "errorWord": "原文中的错误词",
      "correctWord": "正确词",
      "reason": "错误类型（如：错别字、用词不当、语法错误等）"
    }
  ]
}`

    const response = await axios.post('http://localhost:3002/api/ollama/generate', {
      model: 'qwen2:7b',
      prompt: promptText,
      temperature: 0.1,
      max_tokens: 4096,
    ...
    try {
      ...
      const validatedErrors = validateAndLocateErrors(rawErrors)

      // 3. 合并后端规则错误和AI错误 (优先使用后端规则)
      const finalErrors = [...backendErrors]
      
      // 过滤掉与后端规则重叠的AI错误
      validatedErrors.forEach(aiErr => {
        const isOverlapping = backendErrors.some(be => 
          (aiErr.position >= be.position && aiErr.position < be.position + be.errorWord.length) ||
          (be.position >= aiErr.position && be.position < aiErr.position + aiErr.errorWord.length)
        )
        if (!isOverlapping) {
          finalErrors.push(aiErr)
        }
      })

      // 为每个错误添加选择状态（默认全部选中）
      aiErrors.value = finalErrors.map(error => ({
        ...error,
        isSelected: true
      }))
      ...
      if (aiErrors.value.length > 0) {
        errorMsg.value = `✅ AI识别出 ${aiErrors.value.length} 处错别字`
      } else {
        errorMsg.value = '✅ 未发现错别字'
      }
    } catch (e) {
      ...
      // 如果AI请求失败，但有后端规则检查结果，则降级显示后端结果
      if (backendErrors.length > 0) {
        aiErrors.value = backendErrors.map(error => ({
          ...error,
          isSelected: true
        }))
        updateSelectedCorrection()
        errorMsg.value = `✅ AI识别出 ${backendErrors.length} 处错别字`
      } else {
        errorMsg.value = `AI服务请求失败：${e.message || '请确保Ollama已运行'}`
        selectedCorrectedText.value = originText.value
      }
    } finally {
      isChecking.value = false
    ...
    if (backendErrors.length > 0) {
      ...
      errorMsg.value = `检查失败：${e.message || '未知错误'}`
      selectedCorrectedText.value = originText.value
    }
    isChecking.value = false
  }
...
const manualCheck = () => {
  ...
  const errorPatterns = [
    { pattern: /农甜/g, correct: '农田', reason: '错别字' },
    { pattern: /开恳/g, correct: '开垦', reason: '错别字' },
    { pattern: /按装/g, correct: '安装', reason: '错别字' },
    { pattern: /工续/g, correct: '手续', reason: '错别字' },
    { pattern: /工长/g, correct: '手续', reason: '错别字' },
    { pattern: /侵站/g, correct: '侵占', reason: '错别字' },
    { pattern: /耕地补平衡/g, correct: '耕地占补平衡', reason: '成分残缺' },
    { pattern: /集中利用/g, correct: '集约利用', reason: '用词不当' },
    { pattern: /保率/g, correct: '保护率', reason: '成分残缺' },
    { pattern: /附性/g, correct: '刚性', reason: '用词不当' },
    { pattern: /被耕地/g, correct: '耕地', reason: '用词不当' },

    // 通用常见错别字
    { pattern: /因该/g, correct: '应该', reason: '错别字' },
    { pattern: /以经/g, correct: '已经', reason: '错别字' },
    { pattern: /必需/g, correct: '必须', reason: '用词不当' },
    { pattern: /反映/g, correct: '反应', reason: '用词不当' },
    { pattern: /权利/g, correct: '权力', reason: '用词不当' },
    { pattern: /工做/g, correct: '工作', reason: '错别字' },
    { pattern: /部置/g, correct: '布置', reason: '错别字' },
    { pattern: /寻问/g, correct: '询问', reason: '错别字' },
    { pattern: /蓝布山/g, correct: '蓝布衫', reason: '错别字' },
    { pattern: /在次/g, correct: '再次', reason: '错别字' },
    { pattern: /哪道/g, correct: '那道', reason: '错别字' },
  ]
  ...
  errorPatterns.forEach(pattern => {
    ...
    while ((match = regex.exec(originText.value)) !== null) {
      manualErrors.value.push({
  ...
  // 去重
  manualErrors.value = manualErrors.value.filter((error, index, self) =>
      index === self.findIndex(e => e.position === error.position)
  )

  // 生成手动修正文本
  manualCorrectedText.value = applyCorrectionsToText(manualErrors.value)

  if (manualErrors.value.length > 0) {
    errorMsg.value = `手动检查发现 ${manualErrors.value.length} 处可能错误`
  } else {
    errorMsg.value = '手动检查未发现预设的常见错误'
    manualCorrectedText.value = originText.value
  }
}
...
const getErrorContext = (position, length) => {
  ...
  return `${beforeError}[${errorText}]${afterError}`
}
...
const getCharErrorClass = (position) => {
  ...
  } else if (showManualSection.value) {
    // 只显示手动检查的错误
    for (const error of manualErrors.value) {
      if (position >= error.position &&
          position < error.position + error.errorWord.length) {
        return 'manual-error-char'
      }
    }
  }
...
const getCharTooltip = (position) => {
  if (showAISection.value) {
    ...
    for (const error of manualErrors.value) {
      if (position >= error.position &&
          position < error.position + error.errorWord.length) {
        return `手动检查：${error.errorWord} → ${error.correctWord} (${error.reason})`
      }
    }
  }
...
// 应用手动修正
const applyManualCorrections = () => {
  if (manualCorrectedText.value) {
    originText.value = manualCorrectedText.value
    manualErrors.value = []
    manualCorrectedText.value = ''
    showManualSection.value = false
    errorMsg.value = '已应用手动修正'
...
.manual-error-char {
  color: #f39c12;
  background: linear-gradient(135deg, rgba(243, 156, 18, 0.2), rgba(243, 156, 18, 0.3));
  font-weight: 600;
  ...
  padding: 0 1px;
  margin: 0 1px;
  animation: manualErrorPulse 2s infinite;
  z-index: 2;
}

@keyframes manualErrorPulse {
  0%, 100% {
    opacity: 1;
